Feature: User
  ...
  Scenario: POST - Membuat data post baru (negatif)
    Given I set post api endpoints
    When I send POST HTTP requestt
    Then I receive valid HTTP response code 201
    And I receive invalid data for new user
  ...
  Scenario: GET - Mendapatkan data post berdasarkan ID (negatif)
  ...
  Scenario: PUT - Mengubah data post berdasarkan ID (negatif)
